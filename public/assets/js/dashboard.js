...
import { formatNumber, formatPercent, classifyValue } from './modules/formatters.js';
...
async function carregarSessaoDashboard() {
  try {
    ...
    try {
      ...
      if (error.message.includes('401') || error.message.includes('Sessão inválida') || error.message.includes('Não autenticado')) {
